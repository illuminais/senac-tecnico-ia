...
| `/api/admin/painel` | GET | JWT admin | query `trimestre` (obrigatório) + `turma` (opcional, fallback pra turma `status='ativa'` única, senão `400`); `[{codigo, uc, descricao, avaliacoes:[{slug,titulo}], totalAlunos, corrigidos}]` — todos os indicadores do mapa curricular do trimestre (por `ano_curso`, não por turma), inclusive sem nenhuma avaliação vinculada (CA2); `avaliacoes` inclui vínculos de qualquer trimestre da própria avaliação (sprint 03) |
| `/api/admin/grade/:slug` | GET | JWT admin | query `turma` (mesmo fallback); `{avaliacao, avaliacaoTurma:{prazo,prazoLabel,status}, indicadores:[{codigo,uc,descricao}], alunos:[{id,nome,email,entregou,notas:{[codigo]:'A'\|'PA'\|'NA'}}]}` — `alunos` só de `users.turma_id` da turma resolvida (CA8); `notas` de cada aluno só traz indicador com nota lançada, ausência = não avaliado (RF12, sprint 03) |
| `/api/admin/notas` | PUT | JWT admin | body `{notas:[{userId, avaliacaoSlug, indicadorCodigo, valor}]}` — batch upsert (`valor` = `'A'\|'PA'\|'NA'`) ou delete (`valor: null`, volta a "não avaliado", nunca vira NA implícito — CA4); retorna `{ok:true, aplicadas:N}` (sprint 03) |
| `/api/admin/boletim` | GET | JWT admin | query `trimestre` + `turma` (mesmo fallback); `{indicadores:[{codigo,uc,descricao}], alunos:[{id,nome,email,consolidado:{[codigo]:'A'\|'PA'\|'NA'}}]}` — `consolidado` junta todas as notas do aluno×indicador (qualquer avaliação) via `consolidarNota` (max A>PA>NA, CA3); indicador sem nenhuma nota não entra na chave (sprint 03) |
